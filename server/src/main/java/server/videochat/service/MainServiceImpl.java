package server.videochat.service;
...
import server.videochat.domain.Room;
import server.videochat.domain.RoomService;
import server.videochat.util.Parser;
...
public class MainServiceImpl implements MainService {
    ...
    public ModelAndView displayMainPage(final Long id, final String uuid) {
        final ModelAndView modelAndView = new ModelAndView("videochat/main");
    ...
    public ModelAndView displaySelectedRoom(final String sid, final String uuid) {
        ...
        if (parser.parseId(sid).isPresent()) {
            ...
            if(room != null && uuid != null && !uuid.isEmpty()) {
                ...
                // open the chat room
                modelAndView = new ModelAndView("videochat/chat_room", "id", sid);
                modelAndView.addObject("uuid", uuid);
            }
        }
